...
type ScheduleRepository interface {
	...
	SetPayment(ctx context.Context, actor auth.Actor, boundPitchID, bookingID int, payment string) (*ScheduleRow, error)
}
...
func (r *scheduleRepo) SetPayment(ctx context.Context, actor auth.Actor, boundPitchID, bookingID int, payment string) (*ScheduleRow, error) {
	scopeSQL, args := scopePredicate(actor, boundPitchID, 3) // $1=bookingID, $2=payment
	q := fmt.Sprintf(`
		UPDATE bookings b
