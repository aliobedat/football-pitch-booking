...
import { Search, MapPin, Navigation, Loader2, Clock, CalendarDays, AlertTriangle, X } from 'lucide-react';

// ─────────────────────────────────────────────────────────────────────────────
// Availability search — date + start time (+ optional location) → pitches open at
// that start and free from it for ≥ 60 min, nearest-first when location is shared.
// Backend: GET /pitches/availability (Cache-Control: no-store).
//
// This module is the LIFTED logic from the former /availability route, factored so
// it can mount on the discovery page unchanged: a hook (useAvailabilitySearch) owns
// all state + fetch, and two presentational components render the controls and the
// results. Behaviour is identical — the fetch fires ONLY on submit, never on mount.
// ─────────────────────────────────────────────────────────────────────────────
...
export interface UseAvailabilitySearch {
...
// useAvailabilitySearch owns the entire search lifecycle. hasSearched is derived
// from results (null = not searched yet → the page shows its default state).
export function useAvailabilitySearch(): UseAvailabilitySearch {
  ...
  const runSearch = useCallback(async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message ?? 'تعذّر البحث، حاول مجدداً');
      setResults(null);
    } finally {
      setLoading(false);
    }
  ...
  return {
    ...
    status, requestLocation: request,
    results, loading, error, startLabel,
...
// AvailabilitySearchBar — the date/time pickers, search button, and the location
// control with its permission-denied / unavailable fallbacks.
export function AvailabilitySearchBar({ s }: { s: UseAvailabilitySearch }) {
  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_auto] gap-3 items-end">
        <div>
          ...
          <div className="relative">
            ...
            <input type="date" value={s.date} min={ammanToday()} onChange={e => s.setDate(e.target.value)}
        ...
        >
          {s.loading ? <Loader2 size={14} className="animate-spin" aria-hidden /> : <Search size={14} aria-hidden />}
          ابحث عن وقت متاح
        </button>
      </div>
      ...
      <div className="mt-3 flex items-center gap-2 flex-wrap">
        ...
        >
          ...
          {s.status === 'granted' ? 'يتم الترتيب حسب موقعك' : 'الأقرب إليّ'}
        </button>
        {s.status === 'denied' && (
...
export function AvailabilityResults({ s }: { s: UseAvailabilitySearch }) {
  return (
    <div>
      {s.error && (
        <div className="flex items-center gap-2 text-[12.5px] text-red-400 bg-red-500/[0.06] border border-red-500/15 rounded-xl px-4 py-3 mb-4">
        ...
        <>
          <div className="flex items-center justify-between mb-4">
            <p className="text-[12px] text-white/35">
              {s.results.length === 0 ? 'لا نتائج' : `${s.results.length} ملعب متاح`}
            </p>
            <button
              type="button"
              onClick={s.clear}
              className="inline-flex items-center gap-1.5 text-[11.5px] font-semibold text-white/45 hover:text-white/75 transition-colors"
            >
                ...
                >
                  ...
                  <div className="p-4 flex flex-col gap-2 flex-1">
                    ...
                    <div className="mt-auto pt-2 flex items-center gap-1.5 text-[12px] text-emerald-300/90">
                      ...
                      <span>
                        متاح من <span className="font-semibold">{s.startLabel}</span>
